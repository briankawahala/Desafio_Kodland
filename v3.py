...
def update():
    global game_over, game_start, list_gem, list_platforms,\
    gravity, jump, background_music, score, movement_enemy_1, invincibility
...
def hero_movement():
    global gravity, jump, hero_x_velocity, hero_y_velocity, jumped
    if collision_check():
        gravity = 1
        hero.y -= 1
    ...
    if (keyboard.up) and collision_check() and not jumped:
        ...
        clock.schedule_unique(set_stop, 0.2)
...
def random_platafoms():  
    global list_platforms
    valid_platforms = [i for i in range(-320,321,320)]
    first_platform = [i for i in range(160,1121,1)]
    list_platforms[0].x = int(random.choice(first_plataform))
    list_platforms[0].Y = 600
    i = 1
    while True:
        if i == 6:
            break 
        ...
        while list_platforms[i].x <= 0 or list_platforms[i].x >= WIDTH:
            list_platforms[i].x = int(list_platforms[i-1].x + random.choice(valid_plataforms))
        list_platforms[i].y = int(600 - i*120)
